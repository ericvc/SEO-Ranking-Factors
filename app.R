...
rn = c("Backlinks to Site URL (total)",
...
sidebar <- dashboardSidebar(
  ...
  titlePanel(div(img(src="index.jpg", height=55), style="")),
...
body <- dashboardBody(
  ...
  fluidRow(
    tabBox(
      ...
      tabPanel("Page Speed Metrics", "",
               h4(textOutput("PageSpeedTitle")),
               h6(tableOutput("viewPageSpeed")),
               h6(helpText(uiOutput("PageSpeedDesc1", inline = TRUE))),
               h6(helpText(uiOutput("PageSpeedDesc2", inline = TRUE))),
               h6(helpText(uiOutput("PageSpeedDesc3", inline = TRUE))),
               h6(helpText(uiOutput("PageSpeedDesc4", inline = TRUE))),
               h6(helpText(uiOutput("PageSpeedDesc5", inline = TRUE))),
               br(),
               br()
               ),
...
shinyApp(
  ...
  server = function(input, output, session) { 
    ...
    df_out = eventReactive(input$submit, {
